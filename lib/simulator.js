...
Simulator.prototype.get = function(index, t_characteristic, callback, context) {
  ...
  if (!this.gateway.run) {
    // only callback, characteristic_change will refresh
...
Simulator.prototype.characteristic_change = function(index, t_characteristic, objValue) {

  //this.log("Simulator.characteristic_change %s %s %s", index, JSON.stringify(objValue));
  // todo t_characteristic can be removed by displayName
  ...
  if (this.Accessories.length > 0) {
    if (!this.gateway.run || !this.Accessories[index].i_device.type) {
      ...
      //this.log("Simulator.characteristic_change %s %s %s", c, index, this.Accessories[index].i_value[c]);
    }
    // todo avoid multi refresh
    this.refresh(index, t_characteristic);
  }
}

Simulator.prototype.refresh = function(index, t_characteristic) {

  if (index == "all") {
    var data = {"label": {"data_type": "accessory", "index": index}, "accessories": []};
    for( var i = 0; i < this.Accessories.length; i++ ) {
      data.accessories.push(this.extract(i, t_characteristic));
    }
  }
  else {
    var data = {"label": {"data_type": "accessory", "index": index}, "accessory": this.extract(index, t_characteristic)};
  }
  this.sendData(data);
}

Simulator.prototype.extract = function(index, t_characteristic) {
