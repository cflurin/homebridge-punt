...
Simulator.prototype.setValue = function(data) {
  ...
  var name = data.name;
  //this.log.debug("Simulator.onmessage: %s %s %s %s", name, accessories[name].name, t_characteristic, data.value);
  var sc = accessories[name].service.getCharacteristic(Characteristic[t_characteristic]);
  ...
  if( typeof(sc) !== "undefined") {
    switch (sc.props.format) {
      ...
      default:
        // string, tlv8, 
        value = undefined;
        this.log.warn("Simulator.setValue %s %s %s %s", accessories[name].name, t_characteristic, data.value, JSON.stringify(sc.props));
    }
  
    if (typeof(value) !== "undefined" && value !=  accessories[name].i_value[t_characteristic]) {
      //this.log.debug("Simulator.setValue %s %s %s", accessories[name].name, t_characteristic, data.value);
      accessories[name].save_and_setValue("simulator", t_characteristic, value);
    }
  }
  else {
    this.log.warn("Simulator.setValue %s %s undefined, refresh the Browser", accessories[name].name, t_characteristic);
  }
}

Simulator.prototype.get = function(name, t_characteristic, callback, context) {

  //this.log.debug("Simulator.get %s %s", t_characteristic, name);
  if (!gateway.run) {
    callback(null, accessories[name].i_value[t_characteristic]);
  }
}

Simulator.prototype.set = function(name, t_characteristic, value, callback, context) {

  //this.log.debug("Simulator.set %s %s %s", t_characteristic, name, value);
  if (!gateway.run) {
    //accessories[name].i_value[t_characteristic] = value;
    callback();
  }
}

Simulator.prototype.characteristic_change = function(name, objValue) {
  ...
  if (accessories) {
    ...
    //this.log.debug("Simulator.c_change %s %s %s %s", name, accessories[name].name, c, value);
    this.refresh(name, c);
  }
}

Simulator.prototype.refresh = function(name, c) {

  var data = {"label": {"data_type": "accessory"}, "accessories": {}};
  
  if (name == "all") {
    for (var k in accessories) {
      name = accessories[k].name;
      var c = Object.keys(accessories[name].i_value)[0];
      data.accessories[k] = this.extract(name,c);
    }
  }
  else {
...
Simulator.prototype.extract = function(name, c) {
