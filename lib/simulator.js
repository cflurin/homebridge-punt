...
Simulator.prototype.setValue = function(data) {
  ...
  var sc = this.Accessories[index].service.getCharacteristic(this.Accessories[index].Characteristic[t_characteristic]);
  //this.log.debug("Simulator.onmessage: %s %s %s %s %s", index, this.Accessories[index].name, t_characteristic, data.value, JSON.stringify(sc));
  ...
  if( typeof(sc) !== "undefined") {
    switch (sc.props.format) {
      ...
      default:
        // string, tlv8, 
        value = undefined;
        this.log.warn("Simulator.setValue %s %s %s %s", this.Accessories[index].name, t_characteristic, data.value, JSON.stringify(sc.props));
    }
  
    if (typeof(value) !== "undefined" && value !=  this.Accessories[index].i_value[t_characteristic]) {
      //this.log.debug("Simulator.setValue %s %s %s", this.Accessories[index].name, t_characteristic, data.value);
      this.Accessories[index].save_and_setValue("simulator", t_characteristic, value);
    }
  }
  else {
    this.log.warn("Simulator.setValue %s %s undefined, refresh the Browser", this.Accessories[index].name, t_characteristic);
  }
}
...
Simulator.prototype.extract = function(index, c) {
  ...
  //this.log("Simulator.extract %s %s %s", index, a.name, JSON.stringify(a.i_props));
  return i_data;
}
