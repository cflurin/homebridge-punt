...
function Monitor(log, p_config, plugin_name) {
  this.log = log;
...
Monitor.prototype.startServer = function(Accessories) {
...
Monitor.prototype.handleRequest = function(request, response) {
  ...
  for (var i = 0; i < mAccessories.length; i++ ) {
    this.a_vars[i] = mAccessories[i].getVars();
  }
  var res;
  ...
  for( var i = 0; i < this.a_vars.length; i++ ) {
    res += '<tr><td>' + i + '</td><td>'+ this.a_vars[i].name + '</td>';
    if (this.a_vars[i].i_device.type) {
      res += '<td>' + this.a_vars[i].i_device.type +'</td>';
    } else {
      res += '<td></td>';
    }
    res += '<td>' + this.a_vars[i].i_service +'</td>';
    res += '<td>' + JSON.stringify(this.a_vars[i].i_value).replace('{', '').replace('}','') + '</td>' + '</tr>';
  }
  res += '</table>';
  response.write(res);
