...
Accessory.prototype.get = function(t_characteristic, callback, context) {
  ...
  if (context) {
    this.log("Accessory.characteristic_change %s %s", this.index, JSON.stringify(context));
  }
...
Accessory.prototype.characteristic_change = function(objValue) {
  // todo "context": {"keepalive":true, ...
  if (objValue.context) {
    this.log("Accessory.characteristic_change %s %s", this.index, JSON.stringify(objValue.context));
  }
