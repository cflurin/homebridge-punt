...
function Accessory(log, p_config, Service, Characteristic, index, PuntView, Simulator) {
  ...
  this.props = {
    ...
    "save_and_setValue": this.save_and_setValue
  }
...
Accessory.prototype.save_and_setValue = function (value, t_characteristic, trigger) {

  //this.log("Accessory.save_and_setValue %s %s %s", value, t_characteristic, trigger);
  var now = new Date();
...
Accessory.prototype.get = function(t_characteristic, callback, context) {
  ...
  if (this.gateway.run) {
    ...
    if (this.simulator.run) this.Simulator.get(t_characteristic, this.index, callback, context);
  }
}
...
Accessory.prototype.set = function(t_characteristic, value, callback, context) {
  ...
  if (this.gateway.run) {
    ...
    if (this.simulator.run) this.Simulator.set(t_characteristic, this.index, value, callback, context);
  }
}

Accessory.prototype.characteristic_change = function(objValue) {
  // todo "context": {"keepalive":true, ...
  var logmsg = (objValue && objValue.context) ? JSON.stringify(objValue.context).substring(0, 70) : "no context";
  this.log("Accessory.c_change %s %s", this.index, logmsg);
