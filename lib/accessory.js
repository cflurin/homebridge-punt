/*
Accessory WindowZCovering: Wave FIBARO System FGRM-222 Roller Shutter 2
...
Accessory.prototype.predifined_service = function(Service) {
  ...
  for (var i in this.service.characteristics) {
    ...
    this.log("Accessory.p_service %s %s", this.name, c);
    if (c != "Name") {
      //this.log("Accessory.p_service %s %s", this.name, c);
      this.i_value[c] = 0;
      this.allocate(c);
  ...
  // todo characteristics to implement
  var c_undef = [
    "Name",
  ...
  for (var i in this.service.optionalCharacteristics) {
    ...
    this.log("Accessory.p_service optional %s %s", this.name, c);
    if (c_undef.indexOf(c) < 0) {
      //this.log("Accessory.p_service optional %s %s", this.name, c);
      this.i_value[c] = 0;
      this.allocate(c);
      this.setProps(c);
...
Accessory.prototype.allocate = function(c) {
  ...
  this.service
    .getCharacteristic(this.Characteristic[c])
    .on('get', function(callback, context) {self.get(callback, context, this.displayName, self)})
    .on('set', function(value, callback, context) {self.set(value, callback, context, this.displayName, self)});
}
...
Accessory.prototype.get = function(callback, context, displayName, self) {
  ...
  //self.log("Accessory.get %s", t_characteristic);
  if (self.gateway.run) {
...
Accessory.prototype.set = function(value, callback, context, displayName, self) {
  ...
  var c = displayName.replace(/\s/g, "");
  //self.log("Accessory.set %s", t_characteristic);  
  if (self.gateway.run) {
