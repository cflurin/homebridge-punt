...
Accessory.prototype.get = function(callback, context, displayName) {
  ...
  if (gateway.run) {
    ...
    if (simulator.run) Simulator.get(this.name, c, callback, context);
  }
}
...
Accessory.prototype.set = function(value, callback, context, displayName) {
  ...
  if (gateway.run) {
    ...
    if (simulator.run) Simulator.set(this.name, c, value, callback, context);
  }
}
...
Accessory.prototype.characteristic_change = function(objValue) {
  ...
  if (puntview.run) PuntView.characteristic_change(this.name, objValue);
  if (simulator.run) Simulator.characteristic_change(this.name, objValue);
}
