...
Accessory.prototype.get = function(t_characteristic, callback, context) {
  ...
  if (this.gateway.run) {
    ...
    if (this.simulator.run) this.Simulator.get(t_characteristic, this.index, callback);
  }
}
...
Accessory.prototype.set = function(t_characteristic, value, callback, context) {
  ...
  if (this.gateway.run) {
    ...
    if (this.simulator.run) this.Simulator.set(t_characteristic, this.index, value, callback);
  }
}
...
Accessory.prototype.characteristic_change = function(objValue) {
  // todo "context": {"keepalive":true, ...
  if (objValue) {
    this.log("Accessory.characteristic_change %s %s", this.index, JSON.stringify(objValue).substring(0, 70));
  }
