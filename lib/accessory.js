...
Accessory.prototype.configureAccessory = function(accessory) {
  ...
  /* Test
  var info_service = accessory.getService(Service["AccessoryInformation"]);
  this.log.debug("Accessory.configureAccessory %s %s", accessory.displayName, JSON.stringify(info_service));
  var sc = info_service.getCharacteristic(Characteristic.Identify);
  sc.on('set', this.identify.bind(this));
  */
