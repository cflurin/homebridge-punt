...
Gateway.prototype.get = function(t_characteristic, callback, context) {
  ...
  //this.log("Gateway.get %s %s", t_characteristic, this.name);
  ...
  if (!reading[this.service_name][t_characteristic]) {
    if (this.c_undef.indexOf(t_characteristic) < 0) {
      this.log("Gateway.get %s reading.service.characteristic %s undefined", this.name, t_characteristic);
...
Gateway.prototype.set = function(t_characteristic, value, callback, context) {
  ...
  if (!reading[this.service_name][t_characteristic]) {
    if (this.c_undef.indexOf(t_characteristic) < 0) {
      this.log("Gateway.set %s reading.service.characteristic %s undefined", this.name, t_characteristic);
      this.c_undef.push(t_characteristic);
    }
    callback();
    return;  
  ...
  switch (t_characteristic) {
    ...
    case "SmokeDetected":
      if (r_value.match(/Smoke/)) {
        ...
        //this.log("Gateway.get %s %s %s", this.name, t_characteristic, JSON.stringify(match)); 
        p_value = (match[2]) = "ff" ? 
    ...
    default: // todo: error handling
      p_value = r_value;
      this.log.error("Gateway.get %s characteristic %s not found", this.name, t_characteristic);
  }
  return p_value;
}
