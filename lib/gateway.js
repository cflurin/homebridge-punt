...
reading.SmokeSensor = {};
reading.SmokeSensor.SmokeDetected = "alarm";   // FIBARO System FGSD002 Smoke Sensor
  ...
  this.request.get({url: gateway_url}, function(err, response, body) {
    ...
    if (!err && response.statusCode == 200) {
      ...
      if (!this.i_device.type) {
        this.log.debug("Gateway.get_type %s fhem device undefined", this.name);
      }
...
Gateway.prototype.get = function(t_characteristic, callback, context) {
  ...
  if (!reading[this.service_name] || (reading[this.service_name] && !reading[this.service_name][t_characteristic])) {
    if (this.c_undef.indexOf(t_characteristic) < 0) {
      this.log.debug("Gateway.get %s %s %s undefined", this.name, this.service_name, t_characteristic);
  ...
  if (reading[this.service_name][t_characteristic] == "default") {
    callback(null, this.i_value[t_characteristic]);
    return;
  }
  ...
  if (this.i_value[t_characteristic] && this.p_config.gateway.longpoll) {
    ...
    //this.log("Gateway.get reading %s %s", t_characteristic, reading[this.service_name][t_characteristic]);
    var cmd = '{ReadingsVal("' + this.name + '","' + reading[this.service_name][t_characteristic] + '","")}';
    var gateway_url = this.base_url + '/fhem?cmd=' + cmd + '&XHR=1';
    //this.log("get: gateway_url %s", gateway_url);
    ...
    this.request.get({url: gateway_url}, function(err, response, body) {
      ...
      if (!err && response.statusCode == 200) {
        ...
        //this.log("Gateway.get %s %s", this.name, p_value)
...
Gateway.prototype.set = function(t_characteristic, value, callback, context) {
  ...
  if (!this.i_device.type) {
    //this.log("Gateway.set (callback 1) %s %s %s",this.name, t_characteristic, value);
    callback();
    return;
  }
  
  if (context && context.trigger && context.trigger.match(/fhem/g)) {
    //this.log("Gateway.set (callback 2) %s %s %s",this.name, t_characteristic, value);
    callback();
    return;
  }
  
  if (!reading[this.service_name][t_characteristic]) {
    if (this.c_undef.indexOf(t_characteristic) < 0) {
      this.log("Gateway.set %s %s undefined", this.name, t_characteristic);
      this.c_undef.push(t_characteristic);
    }
    callback();
    return;  
...
Gateway.prototype.parsing = function(t_characteristic, r_value) {
  ...
  switch (t_characteristic) {
    ...
    case "SmokeDetected":
      if (r_value.match(/Smoke/)) {
        var match = r_value.match(/(event )(..)/);
        //this.log("Gateway.parsing %s %s %s", this.name, t_characteristic, JSON.stringify(match)); 
        p_value = (match[2]) = "ff" ? 
          this.Characteristic.SmokeDetected.SMOKE_DETECTED : this.Characteristic.SmokeDetected.SMOKE_NOT_DETECTED; 
      }
      else {
        p_value = this.Characteristic.SmokeDetected.SMOKE_DETECTED;   // todo ???
      }
  ...
  switch (expression) {
    ...
    case  ("Switch" + this.name):
      ...
      if(r_value != this.i_value[c]) {
        //this.log("Gateway.poll_parsing: %s %s %s %s", this.name, c, r_value, this.i_value[c]);
    ...
    case ("MotionSensor" + this.name + '-' + reading.MotionSensor.MotionDetected):
      c = "MotionDetected";
      r_value = (dataobj[1] == "closed") ? false : true;
      //this.log.debug("Gateway.poll_parsing %s %s %s", this.name, c, r_value);
      save_and_setValue("fhem", c, r_value);
      break;
    ...
    case ("Lightbulb" + this.name + '-' + reading.Lightbulb.Brightness):
      ...
      if(r_value != this.i_value[c]) {
        //this.log("Gateway.poll_parsing %s dim %s", this.name, r_value);
        save_and_setValue("fhem", c, r_value);
      }
      break;
    ...
    case ("Lightbulb" + this.name + '-' + reading.Lightbulb.Hue):
      ...
      if (h != this.i_value[c]) {
        //this.log("Gateway.poll_parsing %s h %s", this.name, h);
        save_and_setValue("fhem", c, h);
      }
      
      c = "Saturation";
      if (s != this.i_value[c]) {
        //this.log("Gateway.poll_parsing %s s %s", this.name, s);
