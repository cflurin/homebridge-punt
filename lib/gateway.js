...
/*
Accessory SmokeSensor: ZWave FIBARO System FGSD002 Smoke Sensor
...
Gateway.prototype.first_get = function(uuid, t_characteristic, callback) {
  ...
  if (t_characteristic == Object.keys(this.i_value)[0]) {
    this.get_type(t_characteristic, function() {
      this.log.debug("Gateway.first_get %s %s", this.name, t_characteristic, this.i_device.type);
...
Gateway.prototype.get_type = function(c, callback) {
  ...
  this.request.get({url: gateway_url}, function(err, response, body) {
    ...
    if (!err && response.statusCode == 200) {
      ...
      //this.log.debug("Gateway.get_type %s >%s<", this.name, this.i_device.type);
      
      if (this.i_device.type == "") {
        this.log.debug("Gateway.get_type %s device undefined", this.name);
      }
      
      if (this.i_device.type == "EnOcean" && c == "On") {
        this.get_eventMap(callback);
      } else {
        callback();
      }
    } 
...
Gateway.prototype.get_eventMap = function(callback) {
...
Gateway.prototype.setting = function(t_characteristic, value) {
  ...
  switch (t_characteristic) {
    ...
    case "TargetPosition":
      ...
      this.i_value.TargetPosition = value;
      this.i_value.CurrentPosition = value;
...
Gateway.prototype.Longpoll = function(master_accessory, save_and_setValue) {
  ...
  //this.log.debug("Gateway.Longpoll: %s", this.name);

  if (this.longpoll_running) {
    this.log.debug("Gateway.Longpoll running: %s", this.name);
    return;
  }
  this.longpoll_running = true;

  if (this.name == master_accessory) this.log.debug("Gateway.Longpoll start");
  ...
  //this.log.debug("Gateway.Longpoll starting: " + url );
  ...
  this.request.get( { url: url } ).on( 'data', function(data) {
    ...
    if (this.name == master_accessory) this.log("Gateway.Longpoll end") ;

    this.longpoll_running = false;
    setTimeout(function() {this.Longpoll(master_accessory, save_and_setValue)}.bind(this), 2000);
  }.bind(this)).on( 'error', function(err) {
    if (this.name == master_accessory) this.log.error( "Gateway.Longpoll %s", err);
