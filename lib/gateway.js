...
Gateway.prototype.get_type = function(callback) {
  ...
  this.request.get({url: gateway_url}, function(err, response, body) {
    ...
    if (!err && response.statusCode == 200) {
      ...
      if (typeof this.i_device.type === "undefined") {
...
Gateway.prototype.poll_parsing = function (dataobj, save_and_setValue) {
  ...
  switch (expression) {
    ...
    case ("Lightbulb" + this.name + '-' + reading.Lightbulb.Hue):
      ...
      break;
    case ("LightSensor"  + this.name + '-' + reading.LightSensor.CurrentAmbientLightLevel):
      c = "CurrentAmbientLightLevel";
      r_value = parseFloat(dataobj[1].match(/[+-]?\d*\.?\d+/));
      if (r_value == 0) r_value = 0.0001;
      //this.log.debug("Gateway.poll_parsing %s %s %s %s", this.name, c, r_value, this.i_value[c]);
      save_and_setValue("fhem", c, r_value);
      break;
    ...
    case ("WindowCovering" + this.name + '-' + reading.WindowCovering.CurrentHorizontalTiltAngle):
      ...
      case ("SmokeSensor" + this.name + '-' + reading.SmokeSensor.SmokeDetected):
        c = "SmokeDetected";
        r_value = (dataobj[1] == 0) ? 
          this.Characteristic.SmokeDetected.SMOKE_NOT_DETECTED : this.Characteristic.SmokeDetected.SMOKE_DETECTED;
        save_and_setValue("fhem", c, r_value);
